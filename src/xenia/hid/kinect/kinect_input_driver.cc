...
X_STATUS KinectInputDriver::Setup() {
  ...
  XELOGI(
      "KinectInputDriver: No real backend found — using synthetic skeleton.");
  return X_STATUS_SUCCESS;
}
...
bool KinectInputDriver::TryLoadOpenNI2() {
  ...
  pfn_niteUserTrackerReadFrame_ =
      XE_DLSYM(nite_mod, "niteUserTrackerReadFrame");
  pfn_niteUserTrackerFrameRelease_ =
      XE_DLSYM(nite_mod, "niteUserTrackerFrameRelease");
  ...
  int oni_status =
      reinterpret_cast<PfnOniInit>(pfn_oniInitialize_)(2);  // ONI_API_VERSION=2
  ...
  int tc_status = reinterpret_cast<PfnTrackerCreate>(
      pfn_niteUserTrackerCreate_)(&tracker_handle);
  if (tc_status != 0) {
  ...
  nite_tracker_ =
      reinterpret_cast<void*>(static_cast<uintptr_t>(tracker_handle));
...
void KinectInputDriver::MapNiTE2Skeleton(const float positions[][3],
  ...
  lerp_joint(NUI_JOINT_WRIST_RIGHT, NUI_JOINT_ELBOW_RIGHT,
